...
void BinPrint::print(uint32_t val, uint8_t nibbles, bool newline, bool noindent) {
  int8_t    vallength   = 4 * nibbles - 1;
  uint32_t  msb         = 1ul << vallength;
  uint8_t   bufflength  = vallength + nibbles + (uint8_t)newline + (uint8_t)(!noindent);
  while (Serial.availableForWrite() < bufflength) {
    continue;
  }
  if (noindent == true) {
    for (int8_t i = vallength; i >= 0; i--) {
      if ((i & 0x03) == 0x03 && (i != vallength)) {
        Serial.write(' ');
        if ((i & 0x07) == 0x07 && (i != vallength))
          Serial.write(' ');
      }
      if (val & msb)
        Serial.write('1');
