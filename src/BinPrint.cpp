...
void BinPrint::print(uint32_t data, uint8_t nibbles, bool newline, bool noindent) {
  const int8_t  SMALLGAPSMASK = 0x03;             // both the bitmask and the pattern for narrow gaps
  const int8_t  LARGEGAPSMASK = 0x07;             // both the bitmask and the pattern for wide gaps
  int8_t        msbpos        = 4 * nibbles - 1;  // position of MSB (0-31) eg for two nibbles (one byte) it is 7
  uint32_t      msbmask       = 1ul << msbpos;    // bitmask of MSB eg for two nibbles it is 0x00000080
  if (noindent) {
