...
void BinPrint::print(uint32_t data, uint8_t nibbles, bool newline, bool noindent) {
  const int8_t    GAP     = 0x03;             // bitmask for all gaps, both narrow and wide
  const int8_t    WIDEGAP = 0x07;             // bitmask for wide gaps
  const int8_t    MSBPOS  = 4 * nibbles - 1;  // position of MSB (0-31) e.g. for two nibbles (one byte) it is 7
  const uint32_t  MSB     = 1ul << MSBPOS;    // bitmask of MSB e.g. for two nibbles it is 0x00000080
  if (noindent) {
    for (int8_t i = MSBPOS; i >= 0; i--) {
      if ((i & GAP) == GAP && i != MSBPOS) {
        Serial.write(' ');
        if ((i & WIDEGAP) == WIDEGAP && i != MSBPOS)
          Serial.write(' ');
      }
      if (data & MSB)
        Serial.write('1');
